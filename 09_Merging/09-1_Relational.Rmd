...
means your data will be easier to understand and it will be harder to make costly mistakes.
 ...
 At some point, however, we're going to have to merge (or __join__) the table together to produce a single dataframe, and conduct analysis on that dataframe. 
...
4. A __full join__ keeps all observations in `x` and `y`.
...
1. The default, __`by = NULL`__, uses all variables that appear in both tables, the so called natural join. 
