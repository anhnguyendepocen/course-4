...
`gather` and `spread` are being replaced by `pivot_longer` and `pivot_wider` in `tidyr 1.0.0`, which use ideas from the `cdata` package to make reshaping easier to think about. In a future bootcamp, we'll migrate to those functions.
