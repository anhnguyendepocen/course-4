...
#### Challenge 1. Fit two linear regression models from the gapminder data, where the outcome is `lifeExp` and the explanatory variables are `log(pop)`, `log(gdpPercap)`, and `year`. In one model, treat `year` as a numeric variable. In the other, factorize the `year` variable. How do you interpret each model? {-}

```{r}
model1 <- lm(lifeExp ~ log(pop) + log(gdpPercap) + year, data = gap)
model2 <- lm(lifeExp ~ log(pop) + log(gdpPercap) + factor(year), data = gap)
```


#### Challenge 2. Fit a logistic regression model where the outcome is whether `lifeExp` is greater than or less than 60 years, exploring the use of different predictors. {-}

```{r}
library(dplyr)

gap <- gap %>%
  mutate(lifeExp60 = as.logical(lifeExp >= 60))

logit_model <- glm(lifeExp60 ~ log(pop) + log(gdpPercap) + year, family = "binomial", data = gap)
```


#### Challenge 3. Using `stargazer`, format a table reporting the results from the three models you created above (two linear regressions and one logistic). {-}

```{r}
stargazer(model1, model2, logit_model, title = "Challenge Regression Results", type = "text")
```
