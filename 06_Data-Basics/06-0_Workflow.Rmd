...
You should try not to use absolute paths in your scripts, because they hinder sharing: no one else will have exactly the same directory configuration as you. Another way to direct R to something is to give it a **relative pth**.

Relative paths point to something relatively to where we are, rather than from the root of the file system. For example, if our current working directory is `/home/rterman`, then the relatively path `data/un.csv` directs to the full absolute path: `/home/rterman/data/un.csv`
...
Let's make a project togeteher. Click `File > New Project`, then:
...
Now ehenever you refer to a file with a relative path, it will look for it there.

Go ahead and create an new R script and save it inside the project folder. 
